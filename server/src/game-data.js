...
function buildPriorities({ character, phase, archetype, damageTags, utilityTags, defensiveTags, stashSignals }) {
  ...
  if (archetype.primary === "minion") {
    priorities.push({
      id: "minion-scaling",
      title: "Проверить minion scaling",
      action: "В stash и идолах ищи minion damage, minion health/survivability и бонусы к ключевым summon-навыкам.",
      reason: "В skill bar найдены minion/summon сигналы.",
  ...
  if (phase === "endgame") {
    priorities.push({
      ...
      title: "Подготовить endgame checklist",
      action: "Перед точным сравнением предметов зафиксируй цель билда: основной skill, damage tags, защитный слой, blessings и нужные idols.",
      reason: "Персонаж уже в endgame-диапазоне, поэтому случайные замены предметов могут ломать баланс защиты и урона.",
